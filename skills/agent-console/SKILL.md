...
- Left: the domain records from the db (table or cards). One row per entity, status badge, updated_at. Rows the agent changed in the current run get a subtle highlight for 3 seconds.
...
- No OPENAI_API_KEY: the panel shows "Set OPENAI_API_KEY in .env.local to run the agent" and Run is disabled. Everything else works.
