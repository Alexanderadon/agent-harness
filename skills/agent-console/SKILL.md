...
Controls:
- Reset demo: POST /api/reset, then refetch the table. Always present, top right. Reset also clears the agent panel (`stop()` then `setMessages([])` from useChat, registered by the panel in the same small client context as the row highlight; without `stop()` a stream still running when Reset is pressed puts its parts back over the fresh seed): otherwise an Approve card left from the previous run can be pressed after Reset and write into the fresh seed.
- Red line when the db is per-instance memory: the page reads `dbMode()` from lib/db.ts and, when it is 'memory', shows «Демо-база в памяти: изменения не сохраняются между запросами» above the data (see ai-sdk-agent, health endpoint).
