...
5. Agent endpoint: request body validated with zod; prompt length capped (for example 2 000 chars); `stopWhen: isStepCount(6)`; `maxDuration = 60`; per-IP limit of runs per minute answering 429 (in-memory, per instance; README names it basic); returns 400 on invalid input and a JSON error on model failure, never a stack trace. The real cap on key burn is the spend limit set on the OpenAI account, say so in README.
