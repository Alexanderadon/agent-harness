...
## Не ставить
Python, Ollama, Blender, Figma, GUI для баз данных, любые линтеры сверх того, что даёт create-next-app. Ничего из этого не нужно и всё это ест время.

**Docker: по желанию и только с проверкой.** Для сборки и запуска он не нужен. Как второй путь запуска для судьи он даёт плюс к воспроизводимости, но только если `docker build` с шаблоном `templates/Dockerfile` прошёл на этом ноутбуке до 22-го. Не проверен, значит в проект не кладём: Dockerfile, который не собирается, хуже его отсутствия.
